...
func (tr *ResolverStateful) RebuildTrie(
	...
	getAccRot := func(acc common.Hash, incarnation uint64) (common.Hash, error) {
		...
		if !bytes.Equal(root.Bytes(), rootIH.Bytes()) {
			fmt.Printf("%x!=%x, %x\n\n", root, rootIH, root2)
			panic(1)
		}
	...
	if historical {
		...
		err = tr.MultiWalk2(boltDB, startkeys, fixedbits, tr.Walker, getAccRot)
	}
	if err != nil {
		return err
	}
...
func (tr *ResolverStateful) Walker(keyIdx int, isAccount, isIH bool, k, v []byte, accRoot getAccRootF) error {
	if tr.trace {
	...
	if !isIH && !isAccount && tr.accAddrHashIsValid && !bytes.HasPrefix(k, dbutils.GenerateStoragePrefix(common.BytesToHash(tr.accAddrHash), tr.a.Incarnation)) {
	...
	if len(v) > 0 {
		...
		if tr.curr.Len() > 0 {
			...
			if tr.wasIH {
				...
				storageHash, err := accRoot(common.BytesToHash(tr.accAddrHash), tr.a.Incarnation)
				if err != nil {
					return err
				}
				...
				data = &tr.accData
				//if tr.wasAccount {
				//	// will call GenStructStep for this acc after all storage (because need calculate acc.root)
				//} else {
				//	tr.leafData.Value = rlphacks.RlpSerializableBytes(tr.value.Bytes())
				//	data = &tr.leafData
				//}
...
func (tr *ResolverStateful) MultiWalk2(db *bolt.DB, startkeys [][]byte, fixedbits []uint, walker func(keyIdx int, isAccount, isIH bool, k, v []byte, accRoot getAccRootF) error, getAccRoot getAccRootF) error {
	if len(startkeys) == 0 {
		return nil
	}
	...
	err := db.View(func(tx *bolt.Tx) error {
		...
		for k != nil || ihK != nil {
			...
			if !isIH {
				if err := walker(rangeIdx, isAccount, isIH, k, v, getAccRoot); err != nil {
					return err
				}
				k, v = c.Next()
				continue
			...
			if err := walker(rangeIdx, isAccount, isIH, ihK, ihV, nil); err != nil {
