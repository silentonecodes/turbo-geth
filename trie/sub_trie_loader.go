...
func (stl *SubTrieLoader) LoadSubTries(db ethdb.Database, blockNr uint64, rl1, rl2 RetainDecider, dbPrefixes [][]byte, fixedbits []int, trace bool) (SubTries, error) {
	return stl.LoadFromFlatDB(db, rl1, rl2, dbPrefixes, fixedbits, trace)
}

func (stl *SubTrieLoader) LoadFromFlatDB(db ethdb.Database, rl1, rl2 RetainDecider, dbPrefixes [][]byte, fixedbits []int, trace bool) (SubTries, error) {
	loader := NewFlatDbSubTrieLoader()
	if err1 := loader.Reset(db, rl1, rl2, dbPrefixes, fixedbits, trace); err1 != nil {
